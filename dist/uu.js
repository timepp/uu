...
export function enableFontAwesome(cdnUrl) {
...
export function createElement(parent, tagName, classes = [], child, style = {}, attributes = {}) {
...
export function createButton(parent, classes = [], child, onclick = () => { }) {
    const button = createElement(parent, 'button', classes, child);
    button.onclick = onclick;
    return button;
}
export function createCheck(parent, classes = [], labelText, checked = false, onChange = (checked) => { }) {
...
export function createCheckBtn(parent, classes = [], labelText, accentColor, checked = false, onChange = (checked) => { }) {
    ...
    const updateStyle = () => {
        if (accentColor)
            label.style.backgroundColor = checkbox.checked ? accentColor : '';
    };
    checkbox.checked = checked;
...
export function createTable(parent, props = [], classes = [], styles = {}) {
...
export async function asyncGet(fn) {
...
*/
export function asyncCallFunctionWithProgress(fn, hint = 'Please wait...') {
    setTimeout(async () => {
        callAsyncFunctionWithProgress(() => asyncGet(fn), hint);
    }, 0);
}
export async function callAsyncFunctionWithProgress(fn, hint = 'Please wait...') {
...
export function createJsonView(content, customColors = []) {
...
export function createLargeJsonView(content) {
    const main = createElement(null, 'pre', []);
    const spans = highlightText(content, [[/"[^"]+":/g, 'blue'], [/…[0-9]+ more (chars|items)…/g, 'red']]);
    main.append(...spans);
    return main;
}
export function showDialog(title, content = undefined, options = {}, onCreate) {
...
export function showInDialog(title, content, actions = ['Close']) {
    return showDialog(title, content, {
        actions,
        softDismissable: false
    });
}
let informationExtractor = null;
export function setInformationExtractor(extractor) {
    informationExtractor = extractor;
}
let globalEntityParser = null;
export function setGlobalEntityParser(parser) {
    globalEntityParser = parser;
}
export async function showJsonResult(title, content, parser) {
    const obj = typeof content === 'string' ? JSON.parse(content) : content;
    const fullText = JSON.stringify(obj, null, 2);
    const entityParser = (parser || globalEntityParser) ? (path, value) => {
        if (parser) {
...
export function showGeneralText(title, content) {
    const pre = createElement(null, 'pre');
        ...
        for (const [i, item] of this.items.entries()) {
            ...
            if (interactive) {
                ...
                chip.ondrop = (evt) => {
                    ...
                    if (Number.isNaN(from) || from < 0 || from >= this.items.length || from === i)
                        return;
                    const rect = chip.getBoundingClientRect();
                    const insertAfter = evt.clientX > rect.left + rect.width / 2;
...
export function visualizeObject(obj, cfg = {}) {
...
export function createFoldedString(content, maxLength) {
...
export function showAll(collection) {
    collection.forEach((el) => {
        el.style.display = '';
    });
}
export function hideAll(collection) {
    collection.forEach((el) => {
...
export function getStringColor(str, s = 100, l = 90) {
...
export function syncExistence(element, shouldExist) {
    if (!shouldExist) {
        element.remove();
    }
}
...
export function createToggleBar(values, value, onNewValue) {
...
export function associateDropdownActions(elem, actions) {
    const e = elem;
    e.onclick = async (evt) => {
...
export function createLoadingSpinner(parent, size = '2rem', color = 'primary') {
...
export function createFoldableArea(parent, title, content, initiallyFolded = true) {
    ...
    async function reloadContent(refresh) {
        if (!content)
            return;
...
class CodeMirrorLoader {
    ...
    static async getModules() {
        if (!this.modules) {
            ...
            const [state, view, lang, json, search,] = await callAsyncFunctionWithProgress(() => Promise.all([
...
export async function createCodeMirrorJsonViewer(obj, options = {}) {
    ...
    const commentDecorations = Decoration.set(visulizers.map(v => v.marker.range(v.start, v.end)));
...
export async function createCodeMirrorJsonEditor(initialText) {
...
export async function createMarkdownViewer(markdownText) {
...
export async function createChart(parent, width, height, config) {
    if (!chartJsModule) {
...
export function guessDataType(data) {
...
export async function renderDataInsights(info) {
    ...
    function toDateTimeLabel(t, unit) {
        const d = new Date(t);
        const y = d.getFullYear();
        const m = `${d.getMonth() + 1}`.padStart(2, '0');
        const day = `${d.getDate()}`.padStart(2, '0');
    ...
    function floorDateToBoundary(dt, unit) {
        const d = new Date(dt);
        d.setSeconds(0, 0);
        if (unit === 'minute')
            return d;
