 ...
 */
export function parseTimeRange(s, timeZoneOffset) {
    const input = s.trim();
    if (!input) {
    ...
    function normalizeSpaceLower(text) {
        return text.trim().toLowerCase().replace(/\s+/g, ' ');
    }
    function makeUtcDate(year, month, day, hour, minute, second, ms = 0) {
    ...
    function addMonthsClampedUtc(base, deltaMonths) {
    ...
    function parseExactTimeUtc(text, now) {
        const t = text.trim();
        if (/^now$/i.test(t)) {
    ...
    function applyAdjustmentUtc(base, sign, amount, unit) {
    ...
    function parseTimeSpecUtc(text, now) {
        const t = text.trim();
        ...
        if (m[2] && m[3] && m[4]) {
            d = applyAdjustmentUtc(d, m[2], Number(m[3]), m[4]);
        }
        return d;
    }
    ...
    function toUnitRangeUtc(anchor, unit) {
        if (unit === 'day') {
    ...
    function parseRangeSpecUtc(text, now) {
        const raw = text.trim();
 ...
 */
export function adjustTimeUtc(base, amount, unit) {
    function addMonthsClampedUtc(base, deltaMonths) {
        const year = base.getUTCFullYear();
        const month0 = base.getUTCMonth();
        const day = base.getUTCDate();
...
function ulc(t, converter, timeZoneOffset) {
...
export function adjustTime(base, amount, unit, timeZoneOffset) {
