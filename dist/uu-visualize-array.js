...
export function visualizeArray(arr, cfg = {}) {
    ...
    // get prop value
    function getPropValue(item, prop, index) {
    ...
    function getActiveViewRenderOption() {
        if (state.renderStyle === 'table')
            return cfg.tableRenderOption;
        if (state.renderStyle === 'tile')
            return cfg.tileRenderOption;
        return cfg.wallRenderOption;
    }
    function createActionArea(item, index) {
    ...
    const generalPropFormatter = (item, prop, dataIndex) => {
    ...
    function renderPropValue(item, prop, index) {
    ...
    function attachItemClickHandler(item, index, element) {
        const viewOption = getActiveViewRenderOption();
    ...
    function tableRenderer(startIndex, endIndex) {
    ...
    function tileRenderer(startIndex, endIndex) {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            if (!card) {
                ...
                for (const [j, prop] of getVisibleProps().entries()) {
                    if (!getPropValue(item, prop, dataIndex))
                        continue;
    ...
    function wallRenderer(startIndex, endIndex) {
    ...
    const renderStyleToIndex = (s) => renderStyles.indexOf(s);
    const indexToRenderStyle = (i) => renderStyles[i];
    ...
    const viewToggle = createToggleBar(enabledRenderStyleIcons, renderStyleToIndex(state.renderStyle), (v) => {
        ...
        if (state.renderStyle === nextRenderStyle)
            return;
    ...
    function applyFilter(s) {
        ...
        if (s === '') {
            ...
            if (isExpr) {
                ...
                try {
                    const filterFunc = new Function('item', funcCode);
    ...
    function syncRegionExistence() {
        ...
        if (toolbar.lastElementChild) {
            toolbar.lastElementChild.classList.remove('me-2');
        }
    }
    ...
    sortBtn.onclick = async () => {
        ...
        state.sortBy = r.map(s => {
            ...
            return { prop, order: fromArrow(order) };
        });
        applySort();
        gotoPage(0);
    };
