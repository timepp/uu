...
import { fa, DraggableSortedContainer, createElement, showDialog, syncDisplay, createButton, callAsyncFunctionWithProgress } from './uu.js';
import * as uc from './uu-components.js';
let autofillProvider = (category) => [];
export function setAutofillProvider(provider) {
    autofillProvider = provider;
}
export function createInputPanel(parent, elements, style = 'table', name = '') {
    const appendHistory = (arr, value, maxSize) => {
    ...
    const loadInputHistory = () => {
        ...
        return { panelHistory: [], elementHistory: {} };
    };
    const history = loadInputHistory();
    const saveInputHistory = (values) => {
    ...
    const valueFetchers = {};
    const valueSetters = {};
    const createInputControl = (parent, element) => {
        const initialVal = (element.initialValue || element.defaultValue);
        const initialArr = (element.initialValue || element.defaultValue);
        switch (element.type) {
            case 'single-line-string':
            case 'number':
                {
                    const input = uu.createElement(parent, 'input', ['form-control']);
            ...
            case 'multi-line-string':
                {
                    const textarea = uu.createElement(parent, 'textarea', ['form-control']);
                    textarea.rows = 4;
            ...
            case 'single-select':
                {
                    const select = uu.createElement(parent, 'select', ['form-select']);
                    element.selectOptions?.forEach(option => {
                        const opt = uu.createElement(select, 'option', [], option);
                        opt.value = option;
                    });
    ...
    const applyElementHistory = (key, value) => {
        if (valueSetters[key]) {
            try {
    ...
    const pickHistory = async (key) => {
        const result = await uu.showDialog(`History for ${key}`, undefined, {
            style: { width: '60vw' },
            actions: ['Cancel']
        }, (elements, finish) => {
...
export async function showInputDlg(title, elements) {
 ...
 */
export function createInputArea(parent, obj, fieldOptions = {}, style = 'table', name = '') {
    const elements = [];
    ...
    const getValues = () => {
        ...
        for (const key in values) {
            parsedValues[key] = parseValue(values[key], obj[key]);
        }
        return parsedValues;
    };
    return {
...
export async function showInputDialog(title, obj, fieldOptions = {}) {
...
export async function showConfirmationDialog(title, text) {
...
export function showInputDialogOld(title, fields) {
    return showDialog(title, undefined, {
        classes: [],
        style: { width: '50vw' },
        actions: ['OK', 'Cancel'],
        softDismissable: false
        ...
        if (fields.length === 1 && !fields[0].multiLine) {
            ...
            firstInput.addEventListener('keydown', async (evt) => {
                const e = evt;
...
export async function prompt(title, tip, initialValue) {
    return showDialog(title, undefined, {
        classes: [],
        style: { width: '50vw' },
        actions: ['OK', 'Cancel'],
...
export async function promptMultiline(title, tip, initialValue) {
    return showDialog(title, undefined, {
        classes: [],
        style: { width: '50vw' },
        actions: ['OK', 'Cancel'],
 ...
 */
export function createInputAreaOld(parent, elements) {
...
export function createDataAreaOld(parent, foldable, params) {
 ...
 */
export function createInputAction(title, actionName, valueId, handler, value) {
...
export function createAutofillInput(title, placeholder, initialValue, valueId = title, handler, btn) {
    ...
    const history = JSON.parse(localStorage.getItem(`input-history-${valueId}`) || '[]');
...
export function showSelection(title, options, cfg = {}) {
    return showDialog(title, undefined, {
        ...
        function onSelectionChange(oldSelection, newSelection) {
        ...
        filter.oninput = () => {
            ...
            for (const [value, elem] of Object.entries(elements)) {
                syncDisplay(elem, value.toLowerCase().includes(v));
            }
        };
